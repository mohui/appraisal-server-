...
export const helpers = {
  ...
  compare: function (source, op, target, context) {
    ...
    if (arguments.length !== 4)
      throw new Error('compare 必须接受四个参数');
    if (op === undefined)
      throw new Error('操作符不能为 undefined,请检查参数中是否含有操作符或操作符是否已被引号包裹');
    if (op === "eq" || op === "=") {
      return toCompare((isNotNullOrEmpty(source) && source === target), context, this);
    } else if (op === "not eq" || op === "!=" || op === '<>') {
      return toCompare((isNotNullOrEmpty(source) && source !== target), context, this);
    } else if (op === ">=") {
      return toCompare((isNotNullOrEmpty(source) && isNotNullOrEmpty(source) && source >= target), context, this);
    } else if (op === ">") {
      return toCompare((isNotNullOrEmpty(source) && isNotNullOrEmpty(source) && source > target), context, this);
    } else if (op === "<=") {
      return toCompare((isNotNullOrEmpty(source) && isNotNullOrEmpty(source) && source <= target), context, this);
      ...
      return toCompare((isNotNullOrEmpty(source) && isNotNullOrEmpty(source) && source < target), context, this);
    } else if (op === "||" || op === "or") {   //可以传入两个变量过来 source 不为空或者 target 不为空
      return toCompare((isNotNullOrEmpty(source) || isNotNullOrEmpty(target)), context, this);
    } else if (op === "&&" || op === "and") {  //source 不为空并且 target 不为空
      return toCompare((isNotNullOrEmpty(source) && isNotNullOrEmpty(target)), context, this);
    } else {
