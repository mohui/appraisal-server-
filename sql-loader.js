...
engine.registerHelper('sep', context =>
  context.data.last ? context.inverse() : context.fn()
);
...
module.exports = function(hbsSource) {
  ...
  (async () => {
    ...
    const xmlNodes = htmlParser
      .parse(hbsSource)
      .childNodes.filter(
        it => it.tagName && ['part', 'sql', 'method'].includes(it.tagName)
      )
      .filter(it => {
        //id为空过滤
      ...
      .filter(
        it => it.childNodes && it.childNodes.length && it.childNodes.length > 0
      )
      .filter((it, index, array) => {
        ...
        if (array.map(a => a.id).indexOf(it.id) !== index) {
          //有重复,输出警告
          console.warn(
            `[SQL] 文件: ${this.resourcePath} 中存在重复id [${it.id}]`
          );
          return false;
        } else return true;
      })
    ...
    const methods = xmlNodes
      ...
      .map(it => ({
        ...
        params: it.attrs.params
          ? it.attrs.params.split(',').map(it => it.trim())
          : [],
    ...
    const resolve = modulePath => {
      let resolved = path
        .relative(this.context, modulePath)
        .replace(/\\/g, '/');
