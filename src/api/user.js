...
export default class User {
  ...
  @validate(
    should.string().required().description("用户名"),
    should.string().required().description("密码"),
  )
  async login(username, password) {
    let users = await knrtDB.execute(`
      ...
      where (username = ? or telephone = ?)
        and passwd = ?`, username, username, crypts.md5(password.toString()));
    //是否查询出结果
    ...
    if (user['status'] !== 1) throw new KatoLogicError('用户处于停用状态', 1002);
    ...
    await knrtDB.execute(`
      ...
      values (?, ?, '', '', '后台登录', ?, ?, ?)`, user['username'], user['token'], user['ipaddress'], user['iparea'], date);
    ...
    // 如果是机构，把机构信息push到数组中
    if (user.level === 5) areaCodeArray.push({code: areaInfo[0]['sys_code'], name: areaInfo[0]['area_name']});
